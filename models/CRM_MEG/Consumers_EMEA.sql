...
,businessunit as (
    select BUSINESSUNITID, NAME from {{ source('CRM_MEG_EMEA', 'BUSINESSUNIT') }}
)
,account as (
    select * from {{ source('CRM_MEG_EMEA', 'ACCOUNT') }}
)
,usergeo as (
    select ATTRIBUTEVALUE, VALUE as val from {{ source('CRM_MEG_EMEA', 'stringmap') }}
...
,source_data as (
    ...
    SELECT
        ...
        acc.NAME as ACCOUNT,
        geo.val as Country
    FROM {{ source('CRM_MEG_EMEA', 'contact') }} c
    INNER JOIN {{ source('CRM_MEG_EMEA', 'SYSTEMUSER') }} crt
