...
,businessunit as (
    select BUSINESSUNITID, NAME from {{ source('CRM_MEG_APAC', 'BUSINESSUNIT') }}
)
,account as (
    select * from {{ source('CRM_MEG_APAC', 'ACCOUNT') }}
)
,usergeo as (
    select ATTRIBUTEVALUE, VALUE as val from {{ source('CRM_MEG_APAC', 'stringmap') }}
...
,source_data as (
    ...
    SELECT
        ...
        acc.NAME as ACCOUNT,
        geo.val as Country
    FROM {{ source('CRM_MEG_APAC', 'contact') }} c
    INNER JOIN {{ source('CRM_MEG_APAC', 'SYSTEMUSER') }} crt
