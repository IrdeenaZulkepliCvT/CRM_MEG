...
systemuser as (
    select * from {{ source('CRM_MEG_APAC', 'SYSTEMUSER') }}
)
, product as (
    select * from {{ source('CRM_MEG_APAC', 'product') }}
)

, productusagestatus as (
    select ATTRIBUTEVALUE, VALUE as val from {{ source('CRM_MEG_APAC', 'stringmap') }}
...
,source_data as (
    ...
    SELECT
        ...
        LEFT JOIN {{ source('CRM_MEG_APAC', 'contact') }} nurse
        ON cond._NTT_NURSENAME_VALUE = nurse.CONTACTID
        LEFT JOIN {{ source('CRM_MEG_APAC', 'ELOGIC_PRODUCTTYPE') }} producttype
