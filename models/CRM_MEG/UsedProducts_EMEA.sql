...
systemuser as (
    select * from {{ source('CRM_MEG_EMEA', 'SYSTEMUSER') }}
)
, product as (
    select * from {{ source('CRM_MEG_EMEA', 'product') }}
)

, productusagestatus as (
    select ATTRIBUTEVALUE, VALUE as val from {{ source('CRM_MEG_EMEA', 'stringmap') }}
...
,source_data as (
    ...
    SELECT
        ...
        LEFT JOIN {{ source('CRM_MEG_EMEA', 'contact') }} nurse
        ON cond._NTT_NURSENAME_VALUE = nurse.CONTACTID
        LEFT JOIN {{ source('CRM_MEG_EMEA', 'ELOGIC_PRODUCTTYPE') }} producttype
